...
class App(customtkinter.CTk):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.combobox = customtkinter.CTkComboBox(self.top_bar, values=["MANUAL", "TIMER", "AUTO AI"], width=200, height=40,font=customtkinter.CTkFont(size=17, weight="bold"),dropdown_font=customtkinter.CTkFont(size=17, weight="bold"),)
        self.combobox.place(relx=0.98, rely=0.5, anchor="e")
    ...
    def start_task(self):
        if self.task_running:
            self.textbox.insert("end", "Task already running.\n")
            return
        ...
        if self.combobox.get() == "MANUAL":
            threading.Thread(target=self.controller_instance.perform_task).start()
            self.textbox.insert("end", "Manual Task Started.\n")
        elif self.combobox.get() == "AUTO AI":
            if PSD.capture_and_classify():
                threading.Thread(target=self.controller_instance.perform_task).start()
                self.textbox.insert("end", "Auto-AI Task Started.\n")
            else:
                self.textbox.insert("end", "Auto-AI Detected Clean Panel.\n")
        else:
            self.textbox.insert("end", "Unknown Task Selected.\n")

    def stop_task(self):
        if not self.task_running:
            self.textbox.insert("end", "No task running to stop.\n")
            return
        ...
        self.textbox.insert("end", "Task Stopped.\n")
